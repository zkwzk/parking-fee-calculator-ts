...
export class FixedFirstXMinutes extends BaseFee {
    ...
    constructor(startTime: string, endTime: string, x: number, feeFirstXMintues: number, y: number, subsequenceChargePerYMinutes: number) {
    ...
    calculateCost: (fit: FitResult) => number = (fit: FitResult) : number => {
        ...
        const startTimeAfterXMins = this.startTime.plusMinutes(this.x);
        ...
        const timeDiff = startTimeAfterXMins.until(fit.endTime!, ChronoUnit.MINUTES);
        
        const chargeNumberOfYMintues = Math.ceil(timeDiff / this.y);

        return this.feeFirstXMintues + chargeNumberOfYMintues * this.subsequenceChargePerYMinutes;
    }
}